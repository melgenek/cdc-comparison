...
const HASH_BITS: usize = std::mem::size_of::<u64>() * 8;

pub struct GoogleStadiaCdc {
    threshold: u64,
}

impl GoogleStadiaCdc {
    pub fn new(chunk_sizes: ChunkSizes) -> Self {
        Self { threshold: u64::MAX / (chunk_sizes.avg_size() as u64 - chunk_sizes.min_size() as u64 + 1) }
    }
}
